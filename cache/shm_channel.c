...
shm_segment_t* shm_init(int nsegments, size_t segsize) {
    ...
    for (int i = 0; i < nsegments; i++) {
        ...
        sem_t *sem_write = SHM_SEM_WRITE(seg_ids[i].proxy_ptr);
        if (sem_init(sem_write, 1, 0) == -1) {
            fprintf(stderr, "Unable to initialize sem_write %u: %s (errno:%d)\n",
                    i, strerror(errno), errno);
            munmap(seg_ids[i].proxy_ptr, segsize);
            // Clean up previously created segments
            for (int j = 0; j < i; j++) {
                sem_t *prev_data = SHM_SEM_WRITE(seg_ids[j].proxy_ptr);
                sem_t *prev_done = SHM_SEM_READ(seg_ids[j].proxy_ptr);
        ...
        sem_t *sem_read = SHM_SEM_READ(seg_ids[i].proxy_ptr);
        if (sem_init(sem_read, 1, 1) == -1) {
            fprintf(stderr, "Unable to initialize sem_read %u: %s (errno:%d)\n",
                    i, strerror(errno), errno);
...
void shm_cleanup(shm_segment_t *seg_ids, int nsegments) {
    ...
    for (int i = 0; i < nsegments; i++) {
        ...
        if (seg_ids[i].proxy_ptr != NULL && seg_ids[i].proxy_ptr != MAP_FAILED) {
            sem_t *sem_data_ready = SHM_SEM_WRITE(seg_ids[i].proxy_ptr);
            sem_t *sem_read_done = SHM_SEM_READ(seg_ids[i].proxy_ptr);
...
int shm_reset_segment(shm_segment_t *seg) {
    ...
    sem_t *sem_data_ready = SHM_SEM_WRITE(seg->proxy_ptr);
    sem_t *sem_read_done = SHM_SEM_READ(seg->proxy_ptr);
